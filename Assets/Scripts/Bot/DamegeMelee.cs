...
public class DamegeMelee : MonoBehaviour
{
    ...
    public float attackRange = 1.8f;  // bán kính vung
    public float attackRadius = 0.6f; 
    public float cooldown = 1f;     // delay giữa 2 cú chém
    public LayerMask enemyMask;          // layer Enemy

    [Header("Refs")]
    public Transform swingOrigin;      // điểm gốc chém (đặt ở trước tay/dao)
           // optional animation
    ...
    // Gọi khi bấm chuột trái
    public void Attack()
    {
        if (canAttack) 
        { Collider[] hits = Physics.OverlapSphere(swingOrigin.position + transform.forward * attackRange * 0.5f, attackRadius,
            enemyMask, QueryTriggerInteraction.Ignore);
            if (hits.Length > 0) { StartCoroutine(DoAttack(hits)); } 
        }
    }
